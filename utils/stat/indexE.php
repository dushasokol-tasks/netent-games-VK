...
$myQueryText = "SELECT * FROM ns.social_nsStats ORDER BY ts ASC";
...
foreach ($statByTerm as $e => $v) {
	...
	if (($i - $a) < $_GET['period']) {
		...
		foreach ($v as $e1 => $v1) {
			if ($v1['spins'] > 0) $active_players++;
			echo $e1 . " (" . $v1['pr'] . "):" . round($v1['in']) . " / " . round($v1['out']) . " = " . round(($v1['in'] - $v1['out'])) . " S=" . $v1['spins'] . "<br> ";

			if ($i == $a) {
				//	    $myQueryText="SELECT * FROM ns.social_terminals where id=$e1";
				$myQueryText = "SELECT * FROM ns.common where sessionId='$e1'";

				$q = mysql_query($myQueryText);
				if (@mysql_num_rows($q) != 0) {
					$f = @mysql_fetch_array($q);
					$pidor = $f['betted_full'] - $f['winned_full'];
		...
		echo "<br><b>Acive players: $active_players </b><hr>";
	}
	$a++;
}
