...
if (!empty($_POST["req"])) {
  $req = $_POST["req"];
} else if (!empty($_GET["req"])) {
  $req = $_GET["req"];
} else {
  sendToClient(getResultXmlString("empty_request"));
  exit();
}

$decoded = base64_decode($req);
...
if (!$xml) {
  sendToClient(getResultXmlString("parse_error"));
  exit();
}
...
sendToClient(getResultXmlString("unknown_command"));
exit();
...
function getSrvCmdXml($id, $termCred, &$outCmdId)
{
  $outCmdId = 0;
  $resXml = "";
  $q = @mysql_query("SELECT * FROM srvcmd WHERE term_id='$id' AND sended='0' order BY cmd DESC");
  if (@mysql_num_rows($q) == 0) {
    ...
    $denom  = extractCsvParam($paramsStr, "denom");
    if ($denom == 0 or $denom == '' or !isset($denom)) $denom = 1;
    $termCred = $termCred / $denom;

    if ($f['cmd'] == "extCredit" and $termCred < 10 and $termCred >= 0) {
      $cmdId = $f['id'];
      $f['cmd'] = 'svaddcred1';
      $qu = "UPDATE srvcmd SET cmd='svaddcred1' WHERE id='$cmdId'";
      $q = @mysql_query($qu);
      //	userlog2("231"," change: credit=".$termCred);
      $resXml = getResultXmlString("ok");
    } elseif ($f['cmd'] != "extCredit")
    ////
    {
      $cmdid = $f["id"];
      $termid = $f["term_id"];
...
function cmd_updategamedata($xml)
{
  ...
  if (getTerminalData($hwid, $terminalData)) {
    if (isTerminalBanned($terminalData)) {
      ...
      $cmdId = 0;
      $resXml = getSrvCmdXml($id, $termCred, $cmdId);
      if ($cmdId != 0) {
...
function cmd_svaddcred1ack($xml, $xmlString)
{
  ...
  $status = $ECLIENT_OK;
  if (isset($xml->status["val"])) {
    # WebClient 1.0 not have "status" param
    $status = intval($xml->status["val"]);
  }
  ...
  if (getTerminalData($hwid, $terminalData)) {
	...
	{
//	    $myQueryText="UPDATE rep_contracts SET balls_delta='0' WHERE number='$client_id';";
//	    $q = @mysql_query ($myQueryText) or die ("mysql_query: " . mysql_error());
	    $myQueryText=date(h.":".i." ".d.".".m,time()).";action=NEWADD;userId=".$owner_id.";clubId=".$clubId.";client_id=".$client_id.";last_added_id=".$terminalData['lastClient'].",";
	    $myQueryText="log=concat(log,'".$myQueryText."')";
	    $myQueryText="UPDATE rep_departments SET $myQueryText WHERE id='$clubId';";
	    $q = @mysql_query ($myQueryText) or die ("mysql_query: " . mysql_error());
...
*/
        ...
        #$adv = $denom * $addval; # Denomination use
        #$params = "addval=$adv;denom=$denom;origval=$addval";

        #|| (intval($afterAddCredits) != (intval($beforeAddCredits) + $addval))
        if ($status != $ECLIENT_OK) {
          # ADD CREDIT ERROR
          debugLog("ADD_CREDIT_ERROR. ClubId:" . $clubId . " TermId:" . $id . " client_status=$status before=" . $beforeAddCredits . " after=" . $afterAddCredits . " add=" . $addval);
          debugLog("XML from client: " . $xmlString);

          $terminalData["status"] = $ECLIENT_STATUS_ADD_ERROR;
          setTerminalDataById($id, $terminalData);

          # Write to log
          $msg = "oper=addcredit_error;client_status=$status;addval=$origval;denomaddval=$addval;$statString";
          $tm = time();
          ...
          $terminalData["status"] = $ECLIENT_STATUS_ADDED;
          ...
          //        if ($jp=='' and $clubId=='185')
          if ($jp == '') {
            $bonus = 'none';
            $myQueryText = "SELECT * FROM rep_departments WHERE id=$clubId;";
            $q = @mysql_query($myQueryText) or die("mysql_query: " . mysql_error());
            $dept = @mysql_fetch_array($q);

            $myQueryText = "SELECT * FROM rep_contracts WHERE number='$client_id';";
            $q = @mysql_query($myQueryText) or die("mysql_query: " . mysql_error());
            ...
            $myQueryText = "UPDATE rep_contracts SET full_in='$fll_in', full_delta='$fll_dt', short_delta='$sh_dt', balls_delta='$bl_dt' WHERE number='$client_id';";
            $q = @mysql_query($myQueryText) or die("mysql_query: " . mysql_error());
            //	    userlog2("$owner_id",$myQueryText." + ".$old);

            //        $old=$dept['trans_num'].", ".$dept['dialy'].";".$dept['djp_start_hour']." = ".date("H");
            ...
            if ($dept['dialy'] > 0) {
              ...
              $myQueryText = "UPDATE rep_departments SET dialy='" . $dept['dialy'] . "', trans_num='" . $dept['trans_num'] . "', steps='" . $dept['steps'] . "' WHERE id='" . $clubId . "'";
              $q = @mysql_query($myQueryText) or die("mysql_query: " . mysql_error());
              //	    userlog2("$owner_id","$myQueryText");
            }

            //	if($bonus=='none' and $clubId=='185')
            if ($bonus == 'none')
            ...
            if ($bonus != 'none') {
              ...
              $ts = time();
              $query_str = "INSERT INTO rep_queue (cmd, params, term_id, user_id, timestamp) VALUES ('$bonus', '$prms', '$id', '$owner_id', '$ts')";
              $q = @mysql_query($query_str) or die('mysql_query error: ' . mysql_error());
            }

            if ($bonus == '3rd' or $bonus == '2nd' or $bonus == '1st') {
              $dept[$bonus] = 0;
              $myQueryText = "UPDATE rep_departments SET  1st='" . $dept['1st'] . "', 2nd='" . $dept['2nd'] . "', 3rd='" . $dept['3rd'] . "', JPdelay='" . $dept['JPdelay'] . "' WHERE id='" . $clubId . "'";
              $q = @mysql_query($myQueryText) or die("mysql_query: " . mysql_error());
            }
          }
...
function cmd_svpaycred1ack($xml, $xmlString)
{
  ...
  $hwid    = $xml->hwid["val"];
  $svCmdId = $xml->cmdid["val"];
  $beforeCredits = $xml->before_credits["val"]; # Credits before
  $afterCredits  = $xml->after_credits["val"];  # Credits after
  ...
  if (getTerminalData($hwid, $terminalData)) {
    if (isTerminalBanned($terminalData)) {
      ...
      if (getSrvCmd($svCmdId, $srvCmdData)) {
        ...
        if ($client_id != $terminalData["lastClient"] and $terminalData["lastClient"] != 0) {
          $myQueryText = "UPDATE rep_contracts SET balls_delta='0' WHERE number='" . $terminalData["lastClient"] . "';";
          $q = @mysql_query($myQueryText) or die("mysql_query: " . mysql_error());
        ...
        if (($status != $ECLIENT_OK) || (intval($afterCredits) != 0)) {
          ...
          # Write to log
          $msg = "oper=paycredit;client_status=$status;payval=$origCredit;denompayval=$credit;owner_id=$owner_id;client_id=$client_id;doc_id=$doc_id;rfid=$rfid;$statString";
          $tm = time();
          writeToCashLog1($clubId, $msg, $tm);
          writeToTerminalLog($id, $msg, $tm);
          ...
          $myQueryText = "UPDATE rep_contracts SET full_out='$fll_out', full_delta='$fll_dt', short_delta='$sh_dt', balls_delta='$bl_dt' WHERE number='$client_id';";
          $q = @mysql_query($myQueryText) or die("mysql_query: " . mysql_error());
          ...
          if ($dept['dialy'] > 0) {
            if ($dept['djp_start_hour'] <= date("H") and $dept['djp_start_hour'] != 0) {
              $dept['trans_num']++;
              $myQueryText = "UPDATE rep_departments SET trans_num='" . $dept['trans_num'] . "' WHERE id='" . $clubId . "'";
              $q = @mysql_query($myQueryText) or die("mysql_query: " . mysql_error());
            }
          }
        }
...
/*
function cmd_get_server_time($xml)
{
  $res = "<root><result res=\"ok\"/>";
